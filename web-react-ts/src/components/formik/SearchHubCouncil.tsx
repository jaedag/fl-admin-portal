...
import { MEMBER_HUBCOUNCIL_SEARCH } from './SearchHubCouncilQueries'

const SearchHubCouncil = (props: RoleBasedSearch) => {
  ...
  const [memberSearch, { error: memberError }] = useLazyQuery(
    MEMBER_HUBCOUNCIL_SEARCH,
    {
      onCompleted: (data) => {
        setSuggestions(data.members[0].hubCouncilSearch)
        return
      },
    }
  )
...
export default SearchHubCouncil
