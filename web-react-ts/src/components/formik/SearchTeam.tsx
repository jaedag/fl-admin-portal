...
import {
  ...
  MEMBER_TEAM_SEARCH,
  COUNCIL_TEAM_SEARCH,
} from './SearchTeamQueries'
import TextError from './TextError/TextError'

const SearchTeam = (props: RoleBasedSearch) => {
  ...
  const [campusSearch, { error: campusError }] = useLazyQuery(
    ...
    {
      onCompleted: (data) => {
        setSuggestions(data.campuses[0].teamSearch)
        return
      },
    }
  )
  const [streamSearch, { error: streamError }] = useLazyQuery(
    ...
    {
      onCompleted: (data) => {
        setSuggestions(data.streams[0].teamSearch)
        return
      },
    }
  )
  const [councilSearch, { error: councilError }] = useLazyQuery(
    ...
    {
      onCompleted: (data) => {
        setSuggestions(data.councils[0].teamSearch)
        return
      },
    }
  )
  ...
  const [memberSearch, { error: memberError }] = useLazyQuery(
    ...
    {
      onCompleted: (data) => {
        setSuggestions(data.members[0].teamSearch)
        return
      },
    }
  )
