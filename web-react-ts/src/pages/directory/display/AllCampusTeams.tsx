...
import { GET_CAMPUS_TEAMS } from '../../../queries/ListQueries'
...
const AllCampusTeams = () => {
  const { clickCard, campusId } = useContext(ChurchContext)

  const { data, loading, error } = useQuery(GET_CAMPUS_TEAMS, {
    variables: { id: campusId },
  })

  const teams = data?.campuses[0].teams
        ...
        <div className="mb-4">
          <Row className="mb-2">
            <Col>
              ...
              >
                <h4 className="text-white">{`${campus?.name}'s Teams`}</h4>
              </Link>
              <Link
