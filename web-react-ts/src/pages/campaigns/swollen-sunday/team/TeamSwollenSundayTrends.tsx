...
import {
  TEAM_SWOLLEN_DETAILS,
  TEAM_SWOLLEN_SUNDAY_GRAPHS,
...
const TeamSwollenSundayTrends = () => {
  ...
  const {
    ...
    loading: teamLoading,
    error: teamError,
  } = useQuery(TEAM_SWOLLEN_DETAILS, {
    variables: {
      teamId,
    },
  })

  const [teamSwollenSundayGraph, { loading }] = useLazyQuery(
    TEAM_SWOLLEN_SUNDAY_GRAPHS,
    {
      onCompleted: (data) => {
        if (!setChurchData) return
        setChurchData(getServiceGraphData(data?.teams[0], 'swellBussing'))
      },
  ...
  return (
    <ApolloWrapper loading={teamLoading} error={teamError} data={teamData}>
      <Container>
        <h6>{church?.name} Team</h6>
...
export default TeamSwollenSundayTrends
