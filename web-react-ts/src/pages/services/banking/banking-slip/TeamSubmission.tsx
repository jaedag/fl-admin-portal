...
const TeamBankingSlipSubmission = () => {
  const { serviceRecordId } = useContext(ServiceContext)
  const navigate = useNavigate()
  const { togglePopup, isOpen } = usePopup()
  const [errorMessage, setErrorMessage] = useState('')

  const { data, loading, error } = useQuery(TEAM_SERVICE_RECORDS, {
    variables: { serviceId: serviceRecordId },
  })
  const team = data?.serviceRecords[0]?.serviceLog?.team[0]
    ...
    <div>
      {isOpen && (
        <ErrorPopup
          ...
          link="/services/team/banking-slips"
        />
      )}

      <ApolloWrapper loading={loading} error={error} data={data && team}>
        <Formik
        ...
        >
          {(formik) => (
            <Container>
              ...
              <HeadingSecondary>{team?.name}</HeadingSecondary>
              <p>
...
export default TeamBankingSlipSubmission
