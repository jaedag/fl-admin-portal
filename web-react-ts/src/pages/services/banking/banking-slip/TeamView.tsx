...
const TeamBankingSlipView = () => {
  const { teamId, clickCard } = useContext(ChurchContext)
  const navigate = useNavigate()
  const { data, loading, error } = useQuery(TEAM_BANKING_SLIP_QUERIES, {
  ...
  return (
    <Container>
      <HeadingPrimary loading={loading}>{team?.name}</HeadingPrimary>

      {data?.teams[0].services.map((service: ServiceRecord, index: number) => {
        if (
...
export default TeamBankingSlipView
