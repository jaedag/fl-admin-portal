...
MATCH (stream:Stream {id:$streamId})-[:HAS]->(council:Council)-[:HAS]->(constituency:Constituency)-[:HAS]->(bacenta:Bacenta)-[:HAS_HISTORY]->(:ServiceLog)-[:HAS_BUSSING]->(bussing:BussingRecord)-[:BUSSED_ON]->(date:TimeGraph {date:date('2022-09-17')})
MATCH (leader:Member)-[:LEADS]->(bacenta)
MATCH (bussing)-[:INCLUDES_RECORD]->(record:VehicleRecord) WHERE record.arrivalTime IS NOT NULL AND bussing.attendance > 7 AND record.vehicle <> "Car"
OPTIONAL MATCH (constituency)-[:IS_SUPPORTED_BY]->(society:BussingSociety)
RETURN DISTINCT date.date as date, stream.name as stream, bacenta.name as bacenta, ("TN"+toString(bacenta.bacentaCode)) as bacentaCode, record.leaderDeclaration as attendance, record.attendance as confirmedAttendance, record.vehicle as vehicle, 
(CASE 
WHEN bacenta.topUp IS NULL THEN round(toFloat(record.vehicleTopUp), 2)
ELSE round(toFloat(bacenta.topUp), 2) END) as topUp, record.vehicleCost as vehicleCost, record.momoNumber as momoNumber, record.comments as comments, record.arrivalTime as arrivalTime, (leader.firstName+ " "+ leader.lastName) as leader, council.name as council, constituency.name as constituency, society.society as society
`
