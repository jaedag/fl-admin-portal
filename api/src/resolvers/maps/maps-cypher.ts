...
export const universityOutreachVenuesSearchByLocation = `
  MATCH (this:Member {id: $id})-[:LEADS|HAS|DOES_ARRIVALS_FOR|IS_ADMIN_FOR*1..6]->(fellowship:Fellowship)
  MATCH (outreachVenue:UniversityVenue)
  WITH outreachVenue, point.distance(point({latitude: fellowship.location.latitude, longitude: fellowship.location.longitude}), point({latitude: outreachVenue.location.latitude, longitude: outreachVenue.location.longitude})) AS distance
  WHERE distance <= 5000
  RETURN outreachVenue, distance ORDER BY distance, outreachVenue.name ASC LIMIT 30
`
