import { nextHigherChurch, rearrangeCypherObject, throwToSentry, } from '../utils/utils';
import { historyRecordString } from './helper-functions';
import servantCypher from './servant-cypher';
import { matchMemberQuery, matchMemberSheepSeekerQuery, matchMemberTellerQuery, } from '../cypher/resolver-cypher';
import { matchMemberFederalMinistryQuery, matchMemberHubQuery, matchMemberMinistryQuery, matchMemberSontaQuery, } from '../cypher/ministry-directory-cypher';
...
export const removeServantCypher = async ({ context, churchType, servantType, servant, church, }) => {
    ...
    const historyLogRes = rearrangeCypherObject(await session.run(servantCypher.createHistoryLog, {
        id: servant.id,
        churchType,
...
export const makeServantCypher = async ({ context, churchType, servantType, servant, args, church, oldServant, }) => {
    const terms = formatting(churchType, servantType);
    const { servantLower } = terms;
    const session = context.executionContext.session();
    // Connect Leader to Church
    const connectedChurchRes = rearrangeCypherObject(await session
        ...
        .catch((e) => throwToSentry(`Error Connecting Church${servantType}`, e)));
    const historyRecordStringArgs = {
        servant,
        servantType,
        oldServant,
    ...
    const serviceLogRes = rearrangeCypherObject(await session
        .run(servantCypher.createHistoryLog, {
        id: servant.id,
        churchType,
        historyRecord: historyRecordString(historyRecordStringArgs),
    })
        .catch((e) => throwToSentry(`Error Creating History Log`, e)));
    if (servantType === 'Leader') {
        await session
            .run(servantCypher.makeHistoryServiceLog, {
            logId: serviceLogRes.id,
        })
            .catch((e) => throwToSentry(`Error Converting History to Service Log`, e));
        await session
            ...
            .catch((e) => throwToSentry(`Error Connecting Service Log`, e));
    }
    else {
        await session
            ...
            .catch((e) => throwToSentry(`Error Connecting History Log`, e));
    }
};
