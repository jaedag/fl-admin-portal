...
import { errorHandling, isAuth, noEmptyArgsValidation, rearrangeCypherObject, throwToSentry, } from '../utils/utils';
import { sendSingleEmail } from '../utils/notify';
import { changePasswordConfig, createAuthUserConfig, deleteAuthUserConfig, getAuthIdConfig, getUserRoles, updateAuthUserConfig, } from '../utils/auth0';
...
import { assignRoles, churchInEmail, directoryLock, parseForCache, parseForCacheRemoval, removeRoles, } from './helper-functions';
...
export const MakeServant = async (context, args, permittedRoles, churchType, servantType) => {
    ...
    if (!servant.auth_id) {
        ...
        await Promise.all([
            ...
            sendSingleEmail(servant, 'FL Servanthood Status Update', undefined, `<p>Hi ${servant.firstName} ${servant.lastName},<br/><br/>Congratulations on your new position as the <b>${churchType} ${servantType}</b> for <b>${churchNameInEmail}</b>.<br/><br/>Once again we are reminding you to go through ${texts.html.helpdesk} to find guidelines and instructions as well as answers to questions you may have</p>${texts.html.subscription}`),
        ]);
    }
    return parseForCache(servant, church, verb, servantLower);
};
export const RemoveServant = async (context, args, permittedRoles, churchType, servantType, removeOnly) => {
    const authToken = await getAuthToken();
    const authRoles = await getAuth0Roles(authToken);
    const terms = formatting(churchType, servantType);
    const { verb, servantLower, churchLower, memberQuery } = terms;
    ...
    const servant = rearrangeCypherObject(servantRes);
    const newServant = rearrangeCypherObject(newServantRes);
    ...
    if (servant[`${verb}`].length > 1) {
        ...
        await Promise.all([
            ...
            sendSingleEmail(servant, 'You Have Been Removed!', undefined, `<p>Hi ${servant.firstName} ${servant.lastName},<br/><br/>We regret to inform you that you have been removed as the <b>${churchType} ${servantType}</b> for <b>${churchInEmail(church)}</b>.<br/><br/>We however encourage you to strive to serve the Lord faithfully in your other roles. Do not be discouraged by this removal; as you work hard we hope and pray that you will soon be restored to your service to him.</p>${texts.html.subscription}`),
        ]);
    ...
    if (rolesToCompare.includes(`${servantLower}${churchType}`) &&
        ...
        // Send a Mail to That Effect
        sendSingleEmail(servant, 'Your Servant Account Has Been Deleted', undefined, `Hi ${servant.firstName} ${servant.lastName},<br/><br/>This is to inform you that your servant account has been deleted from the First Love Admin Portal. You will no longer have access to any data<br/><br/>his is due to the fact that you have been removed as a ${churchType} ${servantType} for ${churchInEmail(church)}.<br/><br/>We however encourage you to strive to serve the Lord faithfully. Do not be discouraged from loving God by this removal; we hope it is just temporary.${texts.html.subscription}`);
        return parseForCacheRemoval(servant, church, verb, servantLower);
    }
    ...
    if (rolesToCompare.includes(`${servantLower}${churchType}`) &&
        roles.length > 1) {
        removeServantCypher({ context, churchType, servantType, servant, church });
        removeRoles(servant, roles, authRoles[`${servantLower}${churchType}`].id, authToken);
