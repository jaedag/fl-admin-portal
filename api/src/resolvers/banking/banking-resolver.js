...
import { permitAdmin, permitLeader, permitLeaderAdmin, permitMe, } from '../permissions';
import { getMobileCode, getStreamFinancials, } from '../utils/financial-utils';
import { isAuth, rearrangeCypherObject, throwToSentry } from '../utils/utils';
import { checkTransactionReference, getLastServiceRecord, initiateServiceRecordTransaction, setTransactionStatusFailed, setTransactionStatusSuccess, setRecordTransactionReference, setRecordTransactionReferenceWithOTP, submitBankingSlip, } from './banking-cypher';
export const checkIfLastServiceBanked = async (serviceRecordId, context) => {
    ...
    const lastServiceResponse = await session
        ...
        .catch((error) => throwToSentry('There was a problem checking the lastService', error));
...
const bankingMutation = {
    BankServiceOffering: async (object, args, context) => {
        ...
        const transactionResponse = rearrangeCypherObject(await session
            .run(checkTransactionReference, args)
            .catch((error) => throwToSentry('There was a problem checking the transactionReference', error)));
        ...
        const cypherResponse = rearrangeCypherObject(await session
            .run(initiateServiceRecordTransaction, {
            auth: context.auth,
            ...args,
        })
            .catch((error) => throwToSentry('There was an error setting serviceRecordTransactionReference', error)));
        ...
        try {
            const paymentResponse = await axios(payOffering).catch((error) => throwToSentry('There was an error with the payment', error?.response?.data?.data ?? error));
            axios(updatePaystackCustomer);
            if (paymentResponse.data.data.status === 'send_otp') {
                const paymentCypherRes = rearrangeCypherObject(await session
            ...
            const paymentCypherRes = rearrangeCypherObject(await session.run(setRecordTransactionReference, {
                id: serviceRecord.id,
                reference: paymentResponse.data.data.reference,
            }));
            return paymentCypherRes.record;
    ...
    SendPaymentOTP: async (object, args, context) => {
        ...
        if (otpResponse.data.data.status === 'pay_offline') {
            const paymentCypherRes = rearrangeCypherObject(await session
                ...
                .catch((error) => throwToSentry('There was an error setting transaction reference', error)));
            return paymentCypherRes.record;
        }
    ...
    args, context) => {
        ...
        const transactionResponse = rearrangeCypherObject(await session
            .run(checkTransactionReference, args)
            .catch((error) => throwToSentry('There was an error checking transaction reference', error)));
        let record = transactionResponse?.record;
        const banker = transactionResponse?.banker;
        if (!record?.transactionReference) {
            record = rearrangeCypherObject(await session
                .run(setTransactionStatusFailed, args)
                .catch((error) => throwToSentry('There was an error setting the transaction', error)));
        ...
        const confirmationResponse = await axios(confirmPaymentBody).catch(async (error) => {
            throwToSentry('There was an error confirming transaction - ', error.response.data.message);
            if (error.response.data.status === false) {
                record = rearrangeCypherObject(await session.run(setTransactionStatusFailed, args));
            }
        });
        ...
        if (confirmationResponse?.data.data.status === 'success') {
            record = rearrangeCypherObject(await session
                ...
                .catch((error) => throwToSentry('There was an error setting the successful transaction', error)));
            record = record.record.properties;
        }
        return {
            id: record.id,
    ...
    SubmitBankingSlip: async (object, args, context) => {
        ...
        const session = context.executionContext.session();
        await checkIfLastServiceBanked(args.serviceRecordId, context).catch((error) => {
            throwToSentry('There was an error checking if last service banked', error);
        });
        const submissionResponse = rearrangeCypherObject(await session
            .run(submitBankingSlip, { ...args, auth: context.auth })
            .catch((error) => throwToSentry('There was an error submitting banking slip', error)));
